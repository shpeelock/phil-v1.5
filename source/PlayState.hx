...
class PlayState extends MusicBeatState
{
	...
	var pit:FlxSprite = new FlxSprite();
	var countdownFinished:Bool = false;
	var philsaidle:FlxSprite = new FlxSprite(233.65, 187.35);
	var philsa:FlxSprite = new FlxSprite(70, 100.2);
	var flashbang:FlxSprite;
	var vignette:FlxSprite;
	var dodgeAllowed:Bool = false;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'philBg':
				var outer:FlxSprite = new FlxSprite(-136, -179.1).loadGraphic(Paths.image('stagePhil/office/philouter', 'phil'));
				add(outer);
				philbg = new FlxSprite(-136, -179.1).loadGraphic(Paths.image('stagePhil/office/philbg', 'phil'));
			...
			case 'lilmanBg':
				...
				var bg:FlxSprite = new FlxSprite(-22.6, -10.15).loadGraphic(Paths.image('stagePhil/lilman/lilmanBg', 'phil'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.4, 0.4);
				add(bg);
				...
				philsaidle.frames = AtlasFrameMaker.construct('philsa');
				philsaidle.animation.addByPrefix('idle', 'idle', 24, true);
				philsaidle.antialiasing = true;
				philsaidle.visible = false;
				philsaidle.x = 70;
				philsaidle.y = 100.2;
				add(philsaidle);

				philsa.frames = AtlasFrameMaker.construct('philsa');
				philsa.animation.addByPrefix('appearomg', 'appear', 24, false);
				philsa.visible = false;
				add(philsa);
			...
			case 'ilBg':
				if (['adumbrare'].contains(SONG.song.toLowerCase()) && FlxG.random.int(0, 4) == 0)
				{
					FlxG.mouse.visible = true;
					bloxiam = new FlxSprite(FlxG.random.int(-500, 100), FlxG.random.int(-100, 500)).loadGraphic(Paths.image('bloxiam', 'phil'));
					bloxiam.setGraphicSize(Std.int(bloxiam.width * 0.5));
					bloxiam.updateHitbox();
					add(bloxiam);
					trace('sup man');
				}

				var ilBg:FlxSprite = new FlxSprite();
				ilBg.screenCenter();
				// ilBg.scale.set()
			...
			case 'ronBg':
				var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('stagePhil/ron-stage-2', 'phil'));
				bg.antialiasing = true;
				add(bg);
		}
		...
		flashbang = new FlxSprite();
		flashbang.makeGraphic(Std.int(FlxG.width * 2), Std.int(FlxG.height * 2), FlxColor.BLACK);
		flashbang.screenCenter();
		flashbang.cameras = [camHUD];
		flashbang.alpha = 0;
		flashbang.blend = OVERLAY;
		add(flashbang);

		vignette = new FlxSprite();
		vignette = new FlxSprite().loadGraphic(Paths.image('vignette', 'phil'));
	...
	function philsIntro()
	{
		FlxG.camera.zoom *= 1.5;
		startCountdown();
		FlxTween.tween(FlxG.camera, {zoom: defaultCamZoom * 1}, 4, {ease: FlxEase.expoInOut});
	}
	...
	function popupnormal():Void
	{
		// i reuse my own code, easy!
		var popupthing:FlxSprite = new FlxSprite(700 + FlxG.random.float(-200, 200), 470 + FlxG.random.float(-300, 100));
		popupthing.scale.set(1.4, 1.4);
		popupthing.frames = Paths.getSparrowAtlas('popups/popup' + FlxG.random.int(1, 11), 'phil');
		popupthing.animation.addByPrefix('open', 'popup open', 24, false);
		popupthing.animation.addByPrefix('close', 'popup close', 24, false);

		popupthing.antialiasing = true;
		popupthing.cameras = [camHUD];

		add(popupthing);

		FlxG.sound.play(Paths.sound('popup', 'phil'));
		popupthing.animation.play('open');
		canPause = false;

		new FlxTimer().start(FlxG.random.float(2.3, 5), function(tmr:FlxTimer)
		{
			popupthing.animation.play('close');
		...
		if (generatedMusic && !inCutscene)
		{
			...
			if (startedCountdown)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					...
					{
						if (daNote.noteType == 'little note' || daNote.noteType == 'hell note' || daNote.noteType == 'Popup Note'
							|| daNote.noteType == 'Death Note')
						{
							// No
						}
						else
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		switch (eventName)
		{
			...
			case 'philsa':
				philsa.animation.play('appearomg', true);
				philsa.visible = true;

				philsa.animation.finishCallback = function(name:String)
				{
					philsa.visible = false;
					philsaidle.visible = true;
					philsa.animation.finishCallback = null;
				}
	...
	function philFlash():Void
	{
		trace('FLASH!!');

		flashbang.alpha = 1;
		FlxTween.tween(flashbang, {alpha: 0}, 10.525, {ease: FlxEase.expoOut});
	}
 ...
 */
	...
	{
		if (SONG.notes[id] == null || curStage == 'vhs')
			return;
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.noAnimation)
			{
				...
				if (note.noteType == 'Popup Note') // Non-evil
				{
					noteMiss(note);
					popupnormal();
				}
	...
	function spinCharacter(?elapsed:Float = 0):Void
	{
		tankAngle += elapsed * tankSpeed;
		tankGround.angle = tankAngle - 90 + 15;
		tankGround.x = tankX + 1500 * Math.cos(Math.PI / 180 * (1 * tankAngle + 180));
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			case 'pitBg':
				if (curBeat % 2 == 0)
				{
					pit.animation.play('idle', true);
				}

			case 'workerBg':
				if (curBeat % 2 == 0)
				{
					philScared.animation.play('idle', true);
				}

			case 'lilmanBg':
				if (curBeat % 2 == 0)
				{
					philsaidle.animation.play('idle', true);
				}
